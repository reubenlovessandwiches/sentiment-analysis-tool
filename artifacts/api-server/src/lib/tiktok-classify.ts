...
import { analyzeTikTokUserContent } from "./analysis";
import { ARCHETYPES } from "./archetypes";

/** Safety cap on how many TikTok users a single classification pass handles. */
const MAX_CLASSIFY_PER_RUN = 500;

/**
 * Classify every TikTok user that has ingested content but no current analysis
 ...
 */
export async function classifyUnanalyzedTikTokUsers(launchedBy: string | null = null): Promise<void> {
  ...
  // `one_running_analyze_tiktok_batch` partial unique index makes the insert
  // an atomic concurrency guard: if another IG pass is already running, this
  // conflicts and returns nothing, so we skip.
  const [job] = await db
    .insert(jobsTable)
    .values({ jobType: "analyze_tiktok_batch", status: "running", total: capped.length, progress: 0, createdBy: launchedBy })
    .onConflictDoNothing()
    .returning();

  if (!job) {
    logger.info("TikTok auto-classification already in progress; skipping");
    return;
  }

  logger.info(
    { jobId: job.id, count: capped.length, totalQueued: targets.length },
    "TikTok classification started",
  );
  ...
  for (const t of capped) {
    try {
      const posts = await db
        .select({ title: tiktokPostsTable.title, text: tiktokPostsTable.text })
        .from(tiktokPostsTable)
        .where(eq(tiktokPostsTable.authorUsername, t.username))
        .limit(30);

      const comments = await db
        .select({ body: tiktokCommentsTable.body })
        .from(tiktokCommentsTable)
        .where(eq(tiktokCommentsTable.authorUsername, t.username))
        .limit(100);

      const result = await analyzeTikTokUserContent(t.displayName, posts, comments);

      await db.transaction(async (tx) => {
        await tx
          .update(tiktokArchetypeScoresTable)
          .set({ isLatest: false })
          .where(and(eq(tiktokArchetypeScoresTable.userId, t.id), eq(tiktokArchetypeScoresTable.isLatest, true)));
        await tx
          .update(tiktokAnalysesTable)
          .set({ isLatest: false })
          .where(and(eq(tiktokAnalysesTable.userId, t.id), eq(tiktokAnalysesTable.isLatest, true)));

        const [analysis] = await tx
          .insert(tiktokAnalysesTable)
          .values({
            userId: t.id,
        ...
        if (scoreInserts.length > 0) {
          await tx.insert(tiktokArchetypeScoresTable).values(scoreInserts);
        }
      });
      ...
      failed++;
      logger.error({ err, username: t.username }, "TikTok classification failed for user");
    }
  ...
  logger.info({ jobId: job.id, classified, failed }, "TikTok classification completed");
}
