...
import { useGetTwitterUser, getGetTwitterUserQueryKey } from "@workspace/api-client-react";
...
export default function TwitterUserProfile() {
  ...
  const { data: profile, isLoading } = useGetTwitterUser(username || "", {
    query: {
      enabled: !!username,
      queryKey: getGetTwitterUserQueryKey(username || ""),
    },
  });
  ...
  const pdfData = {
    ...
    subtitle: `Twitter/X · @${profile.user.username}`,
    stats: [
