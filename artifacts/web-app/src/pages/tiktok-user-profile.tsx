...
import { useGetTikTokUser, getGetTikTokUserQueryKey } from "@workspace/api-client-react";
...
export default function TikTokUserProfile() {
  const { username } = useParams<{ username: string }>();

  const { data: profile, isLoading } = useGetTikTokUser(username || "", {
    query: {
      enabled: !!username,
      queryKey: getGetTikTokUserQueryKey(username || ""),
    },
  });
  ...
  const pdfData = {
    ...
    subtitle: `TikTok · @${profile.user.username}`,
    stats: [
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground font-mono flex-wrap">
            ...
            <span className="font-mono text-xs opacity-70">TT: @{profile.user.username}</span>
