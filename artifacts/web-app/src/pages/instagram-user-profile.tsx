...
import { useGetInstagramUser, getGetInstagramUserQueryKey } from "@workspace/api-client-react";
...
export default function InstagramUserProfile() {
  ...
  const { data: profile, isLoading } = useGetInstagramUser(username || "", {
    query: {
      enabled: !!username,
      queryKey: getGetInstagramUserQueryKey(username || ""),
    },
  });
  ...
  const pdfData = {
    ...
    subtitle: `Instagram · @${profile.user.username}`,
    stats: [
