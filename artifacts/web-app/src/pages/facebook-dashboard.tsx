...
import {
  ...
  useGetFacebookDashboardThemeDetails,
  getGetFacebookDashboardThemeDetailsQueryKey,
} from "@workspace/api-client-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { FacebookUserLink } from "@/components/facebook-user-link";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function ThemeDetailDialog({ label, open, onClose }: { label: string | null; open: boolean; onClose: () => void }) {
  ...
  const { data, isLoading } = useGetFacebookDashboardThemeDetails(params, {
    query: {
      queryKey: getGetFacebookDashboardThemeDetailsQueryKey(params),
      enabled: open && !!label,
    },
  });
...
export default function FacebookDashboard() {
  ...
  const { data, isLoading } = useGetFacebookDashboard({
    query: {
      queryKey: getGetFacebookDashboardQueryKey(),
    },
  });
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap justify-between items-end gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Facebook Intelligence Overview</h1>
          <p className="text-muted-foreground mt-1 font-mono text-sm">FACEBOOK COMMUNITY DYNAMICS ANALYSIS</p>
        </div>
      </div>
