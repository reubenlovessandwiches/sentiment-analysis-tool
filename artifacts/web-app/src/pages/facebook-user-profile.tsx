...
import { useGetFacebookUser, getGetFacebookUserQueryKey } from "@workspace/api-client-react";
...
export default function FacebookUserProfile() {
  const { profileId } = useParams<{ profileId: string }>();

  const { data: profile, isLoading } = useGetFacebookUser(profileId || "", {
    query: {
      enabled: !!profileId,
      queryKey: getGetFacebookUserQueryKey(profileId || ""),
    },
  });
  ...
  const pdfData = {
    ...
    subtitle: `Facebook · ID ${profile.user.profileId}`,
    stats: [
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground font-mono flex-wrap">
            ...
            <span className="font-mono text-xs opacity-70">FB ID: {profile.user.profileId}</span>
