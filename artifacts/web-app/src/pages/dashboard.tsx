...
import { 
  ...
  useGetDashboardThemeDetails,
  getGetDashboardThemeDetailsQueryKey,
...
function ThemeDetailDialog({ label, open, onClose }: { label: string | null; open: boolean; onClose: () => void }) {
  ...
  const { data, isLoading } = useGetDashboardThemeDetails(params, {
    query: {
      queryKey: getGetDashboardThemeDetailsQueryKey(params),
      enabled: open && !!label,
    },
  });
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Elaborated theme descriptions from individual user profiles
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3 mt-2">
          ...
          {!isLoading && data?.items?.length === 0 && (
            <p className="text-sm text-muted-foreground">No detailed descriptions available yet — re-analyse users to populate this.</p>
          )}
          {data?.items?.map((item, i) => (
            <div key={i} className="p-3 rounded-md bg-muted/20 border border-border/50 space-y-1">
              <div className="flex items-center gap-2">
                <RedditUserLink username={item.username} className="text-xs font-mono font-medium" />
              </div>
...
export default function Dashboard() {
  ...
  const { data, isLoading } = useGetDashboard(undefined, {
    query: {
    ...
    return (
      <div className="p-8 space-y-8 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => <Skeleton key={i} className="h-32 w-full glass" />)}
        </div>
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Global Intelligence Overview</h1>
          <p className="text-muted-foreground mt-1 font-mono text-sm">REAL-TIME COMMUNITY DYNAMICS ANALYSIS</p>
        </div>
      </div>
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={stats?.activityByDay}>
                  ...
                  <Line type="monotone" dataKey="posts" stroke="hsl(var(--accent))" strokeWidth={2} dot={false} name="Posts" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <CardContent>
            <div className="space-y-4">
              {stats?.topUsers?.map((u) => (
                <div key={u.user.id} className="flex items-center justify-between p-3 rounded-md bg-muted/20 border border-border/50 hover:border-primary/50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <RedditUserLink username={u.user.username} className="font-medium" />
                      </div>
        ...
        <Card className="glass">
          <CardHeader>
            ...
            <CardDescription>Click any theme to see elaborated descriptions per user</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {stats?.topThemes?.map((t, idx) => (
                ...
                >
                  <div className="flex items-center gap-3 w-full">
                    <span className="font-mono text-xs text-muted-foreground w-4">{(idx+1).toString().padStart(2, '0')}</span>
