...
import {
  ...
  useGetTwitterDashboardThemeDetails,
  getGetTwitterDashboardThemeDetailsQueryKey,
} from "@workspace/api-client-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { TwitterUserLink } from "@/components/twitter-user-link";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function ThemeDetailDialog({ label, open, onClose }: { label: string | null; open: boolean; onClose: () => void }) {
  ...
  const { data, isLoading } = useGetTwitterDashboardThemeDetails(params, {
    query: {
      queryKey: getGetTwitterDashboardThemeDetailsQueryKey(params),
      enabled: open && !!label,
    },
  });
...
export default function TwitterDashboard() {
  ...
  const { data, isLoading } = useGetTwitterDashboard({
    query: {
      queryKey: getGetTwitterDashboardQueryKey(),
    },
  });
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap justify-between items-end gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Twitter Intelligence Overview</h1>
          <p className="text-muted-foreground mt-1 font-mono text-sm">TWITTER COMMUNITY DYNAMICS ANALYSIS</p>
        </div>
      </div>
