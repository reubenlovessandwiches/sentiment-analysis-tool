...
import {
  useListFacebookUsers,
  getListFacebookUsersQueryKey,
...
import { Search, ShieldAlert, X, ChevronRight, ChevronDown, Sparkles } from "lucide-react";
...
function FacebookIdCell({ id }: { id: string }) {
  ...
  return (
    <div className="flex items-start gap-1.5">
      <button
        ...
        aria-label={expanded ? "Collapse Facebook ID" : "Expand Facebook ID"}
        className="mt-px shrink-0 text-muted-foreground hover:text-foreground transition-colors"
      >
        {expanded ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
      </button>
      <span
        title={id}
        className={`font-mono text-xs text-muted-foreground min-w-0 ${
          expanded ? "max-w-[10rem] break-all" : "max-w-[8rem] truncate"
        }`}
      >
        {id}
      </span>
...
export default function FacebookUsers() {
  ...
  const { data, isLoading } = useListFacebookUsers(queryParams, {
    query: { queryKey: getListFacebookUsersQueryKey(queryParams) },
  });
  ...
  const resumeClassify = useResumeFacebookClassification();
    ...
    try {
      ...
      if (res.status === "started") {
        toast({
          ...
          description: `Classifying ${res.pending} unclassified Facebook ${res.pending === 1 ? "user" : "users"}.`,
        });
      } else if (res.status === "already_running") {
        toast({ title: "Already running", description: "A classification pass is already in progress." });
      } else {
        toast({ title: "Nothing to classify", description: "All Facebook entities are already classified." });
      }
      queryClient.invalidateQueries({ queryKey: getListFacebookUsersQueryKey() });
  ...
  return (
    <div className="p-8 space-y-6 h-full flex flex-col">
      <div className="flex flex-wrap justify-between items-end gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Facebook Entity Directory</h1>
          <p className="text-muted-foreground mt-1 font-mono text-sm">INDEX OF CLASSIFIED FACEBOOK PROFILES</p>
        </div>
              ...
              {isLoading ? (
                ...
                data?.users.map((u) => (
                  <tr key={u.user.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                    ...
                    <td className="px-6 py-3 align-top"><FacebookIdCell id={u.user.profileId} /></td>
