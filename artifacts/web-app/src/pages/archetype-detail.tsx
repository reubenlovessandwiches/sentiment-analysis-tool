...
import {
  useGetArchetypeUsers,
  getGetArchetypeUsersQueryKey,
  useListArchetypes,
...
export default function ArchetypeDetail() {
  const [, params] = useRoute("/reddit-archetypes/:key");
  ...
  const { data: users, isLoading } = useGetArchetypeUsers(
    key,
    { minScore: 1, limit: 100 },
    { query: { queryKey: getGetArchetypeUsersQueryKey(key, { minScore: 1, limit: 100 }) } },
  );

  return (
    <div className="p-8 space-y-6">
      <div>
        <Link href="/reddit-archetypes">
                ...
                {isLoading ? (
                  ...
                  users.map((u) => (
                    <tr key={u.user.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`/reddit-users/${u.user.username}`}>
