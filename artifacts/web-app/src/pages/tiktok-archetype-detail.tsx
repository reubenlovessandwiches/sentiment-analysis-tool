...
import {
  useGetTikTokArchetypeUsers,
  getGetTikTokArchetypeUsersQueryKey,
...
export default function TikTokArchetypeDetail() {
  const [, params] = useRoute("/tiktok-archetypes/:key");
  ...
  const { data: users, isLoading } = useGetTikTokArchetypeUsers(
    key,
    { minScore: 1, limit: 100 },
    { query: { queryKey: getGetTikTokArchetypeUsersQueryKey(key, { minScore: 1, limit: 100 }) } },
  );

  return (
    <div className="p-8 space-y-6">
      <div>
        <Link href="/tiktok-archetypes">
                ...
                {isLoading ? (
                  ...
                  users.map((u) => (
                    <tr key={u.user.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`/tiktok-users/${encodeURIComponent(u.user.username)}`}>
