...
import {
  useGetTwitterArchetypeUsers,
  getGetTwitterArchetypeUsersQueryKey,
...
export default function TwitterArchetypeDetail() {
  const [, params] = useRoute("/twitter-archetypes/:key");
  ...
  const { data: users, isLoading } = useGetTwitterArchetypeUsers(
    key,
    { minScore: 1, limit: 100 },
    { query: { queryKey: getGetTwitterArchetypeUsersQueryKey(key, { minScore: 1, limit: 100 }) } },
  );

  return (
    <div className="p-8 space-y-6">
      <div>
        <Link href="/twitter-archetypes">
                ...
                {isLoading ? (
                  ...
                  users.map((u) => (
                    <tr key={u.user.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`/twitter-users/${encodeURIComponent(u.user.username)}`}>
