...
import {
  useGetYoutubeArchetypeUsers,
  getGetYoutubeArchetypeUsersQueryKey,
...
export default function YoutubeArchetypeDetail() {
  const [, params] = useRoute("/youtube-archetypes/:key");
  ...
  const { data: users, isLoading } = useGetYoutubeArchetypeUsers(
    key,
    { minScore: 1, limit: 100 },
    { query: { queryKey: getGetYoutubeArchetypeUsersQueryKey(key, { minScore: 1, limit: 100 }) } },
  );

  return (
    <div className="p-8 space-y-6">
      <div>
        <Link href="/youtube-archetypes">
                ...
                {isLoading ? (
                  ...
                  users.map((u) => (
                    <tr key={u.user.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`/youtube-users/${encodeURIComponent(u.user.username)}`}>
