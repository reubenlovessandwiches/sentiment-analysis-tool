...
import {
  useListTikTokUsers,
  getListTikTokUsersQueryKey,
  useListTikTokArchetypes,
  getListTikTokArchetypesQueryKey,
...
function TikTokHandleCell({ handle }: { handle: string }) {
  return (
    <span
      title={handle}
...
export default function TikTokUsers() {
  ...
  const { data: archetypes } = useListTikTokArchetypes({
    query: { queryKey: getListTikTokArchetypesQueryKey() },
  });
  ...
  const { data, isLoading } = useListTikTokUsers(queryParams, {
    query: { queryKey: getListTikTokUsersQueryKey(queryParams) },
  });
  ...
  const resumeClassify = useResumeTikTokClassification();
    ...
    try {
      ...
      if (res.status === "started") {
        toast({
          ...
          description: `Classifying ${res.pending} unclassified TikTok ${res.pending === 1 ? "user" : "users"}.`,
        });
      } else if (res.status === "already_running") {
        toast({ title: "Already running", description: "A classification pass is already in progress." });
      } else {
        toast({ title: "Nothing to classify", description: "All TikTok entities are already classified." });
      }
      queryClient.invalidateQueries({ queryKey: getListTikTokUsersQueryKey() });
  ...
  return (
    <div className="p-8 space-y-6 h-full flex flex-col">
      <div className="flex flex-wrap justify-between items-end gap-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">TikTok Entity Directory</h1>
          <p className="text-muted-foreground mt-1 font-mono text-sm">INDEX OF CLASSIFIED TIKTOK PROFILES</p>
        </div>
                ...
                data?.users.map((u) => (
                  <tr key={u.user.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                    <td className="px-6 py-3 font-medium">
                      <Link
                        href={`/tiktok-users/${encodeURIComponent(u.user.username)}`}
                        className="hover:text-primary hover:underline transition-colors"
                      >
                        {u.user.displayName}
                      </Link>
                    </td>
                    <td className="px-6 py-3 align-top"><TikTokHandleCell handle={u.user.username} /></td>
