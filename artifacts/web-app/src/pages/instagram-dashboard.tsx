...
import {
  ...
  useGetInstagramDashboardThemeDetails,
  getGetInstagramDashboardThemeDetailsQueryKey,
} from "@workspace/api-client-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { InstagramUserLink } from "@/components/instagram-user-link";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function ThemeDetailDialog({ label, open, onClose }: { label: string | null; open: boolean; onClose: () => void }) {
  ...
  const { data, isLoading } = useGetInstagramDashboardThemeDetails(params, {
    query: {
      queryKey: getGetInstagramDashboardThemeDetailsQueryKey(params),
      enabled: open && !!label,
    },
  });
...
export default function InstagramDashboard() {
  ...
  const { data, isLoading } = useGetInstagramDashboard({
    query: {
      queryKey: getGetInstagramDashboardQueryKey(),
    },
  });
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap justify-between items-end gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Instagram Intelligence Overview</h1>
          <p className="text-muted-foreground mt-1 font-mono text-sm">INSTAGRAM COMMUNITY DYNAMICS ANALYSIS</p>
        </div>
      </div>
