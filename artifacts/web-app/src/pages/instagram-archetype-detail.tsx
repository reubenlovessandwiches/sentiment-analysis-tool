...
import {
  useGetInstagramArchetypeUsers,
  getGetInstagramArchetypeUsersQueryKey,
...
export default function InstagramArchetypeDetail() {
  const [, params] = useRoute("/instagram-archetypes/:key");
  ...
  const { data: users, isLoading } = useGetInstagramArchetypeUsers(
    key,
    { minScore: 1, limit: 100 },
    { query: { queryKey: getGetInstagramArchetypeUsersQueryKey(key, { minScore: 1, limit: 100 }) } },
  );

  return (
    <div className="p-8 space-y-6">
      <div>
        <Link href="/instagram-archetypes">
                ...
                {isLoading ? (
                  ...
                  users.map((u) => (
                    <tr key={u.user.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`/instagram-users/${encodeURIComponent(u.user.username)}`}>
