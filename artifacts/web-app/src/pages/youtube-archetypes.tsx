import { useListYoutubeArchetypes, getListYoutubeArchetypesQueryKey } from "@workspace/api-client-react";
...
export default function YoutubeArchetypes() {
  const { data: archetypes, isLoading } = useListYoutubeArchetypes({
    query: {
      queryKey: getListYoutubeArchetypesQueryKey(),
    },
  });
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Youtube Archetype Models</h1>
        <p className="text-muted-foreground mt-1 font-mono text-sm">BEHAVIORAL CLASSIFICATION TAXONOMY</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {archetypes?.map((arch) => (
          <Link key={arch.key} href={`/youtube-archetypes/${arch.key}`} className="block">
